...
export class ProductScreen extends ControlButtonsMixin(Component) {
    ...
    if (buttonValue === "discount") {
        // Show discount popups instead of changing numpad mode
        ...
        return;
    } else if (["quantity", "price"].includes(buttonValue)) {
    ...
    if (discountType === "percentage") {
        // Apply percentage discount (existing functionality)
        selectedLine.set_discount(value);
    } else {
        // Apply fixed amount discount
        // Get the total price before discount
        const originalPriceUnit = selectedLine.get_unit_price();
        const quantity = selectedLine.get_quantity();
        const totalBeforeDiscount = originalPriceUnit * quantity;

        // Calculate the percentage equivalent of the fixed amount
        // (fixed amount / total before discount) * 100
        if (totalBeforeDiscount <= 0) return;
        const discountPercentage = Math.min(100, (value / totalBeforeDiscount) * 100);

        // Apply the calculated percentage discount
    ...
    if (selectedLine) {
        ...
        // Removed the discount case as we now handle it via popups
    }
}
    ...
    if (last_orderline) {
        this.apply_discount(code.value, "percentage");  // Assume barcode discount is percentage
    }
}
